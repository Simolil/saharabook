  ...
  return (
    <div className="bg-[#FAF7F2] min-h-screen pb-24">
      ...
      <div className="max-w-7xl mx-auto px-4">
         <div className="bg-[#FAF7F2] rounded-[40px] shadow-2xl overflow-hidden border border-[#BA7517]/20">
            <div className="grid grid-cols-5 hidden md:grid-cols-5 md:grid">
               <div className="p-10 bg-gray-50/40 border-r border-b border-[#BA7517]/10 flex items-center">
               ...
               {['Merzouga', 'Zagora', 'Agafay', 'Foum Zguid'].map((d) => (
                 <div key={d} className="p-10 border-b border-r border-[#BA7517]/10 last:border-r-0 text-center">
                    <h3 className="text-xl font-serif font-semibold text-[#0B132B]">{d}</h3>
            ...
            {categories.map((cat, i) => (
               <div key={cat.name} className="grid grid-cols-1 md:grid-cols-5 border-b border-[#BA7517]/10 group">
                  <div className="p-8 md:p-10 bg-gray-50/20 md:border-r border-[#BA7517]/10 flex items-center">
                     <span className="text-xs font-bold text-[#0B132B] uppercase tracking-wider">{cat.name}</span>
                  </div>
                  {['merzouga', 'zagora', 'agafay', 'foumzguid'].map((key) => (
                    <div key={key} className="p-8 md:p-10 md:border-r last:border-r-0 border-[#BA7517]/10 text-center transition-colors group-hover:bg-[#BA7517]/5 flex items-center justify-center">
                       <p className="text-sm font-semibold text-[#0B132B]">{cat[key as keyof Omit<typeof cat, 'name'>]}</p>
                    </div>
                  ))}
               </div>
            ))}
         ...
         <div className="mt-24 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              <div key={rec.dest} className="flex flex-col h-full bg-[#FAF7F2] p-12 rounded-[40px] border border-[#BA7517]/15 shadow-sm">
                 ...
                 <h3 className="text-2xl font-bold text-[#0B132B] mb-6">Choose {rec.dest}</h3>
                 <p className="text-sm text-[#0B132B]/60 mb-8 leading-relaxed flex-1">{rec.desc}</p>
